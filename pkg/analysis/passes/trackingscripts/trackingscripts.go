	...
	for _, content := range moduleJsMap {
		for _, url := range servers {
			if len(url) > 0 && bytes.Contains(content, []byte(url)) {
				pass.ReportResult(pass.AnalyzerName, trackingScripts, "module.js: should not include tracking scripts", fmt.Sprintf("Tracking scripts are not allowed in Khulnasoft plugins (e.g. google analytics). Please remove any usage of tracking code. Found: %s", url))
